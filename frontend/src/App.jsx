...
const api={
  ...
  vaultProjects:(opts={})=>{
    ...
    if(opts.limit)q.set("limit",String(opts.limit));
    const qs=q.toString();
    return fetch(`/api/vault/projects${qs?`?${qs}`:""}`,{headers:api._ah()}).then(r=>api._json(r));
  },
  vaultCreateProject:(payload)=>fetch("/api/vault/projects",{method:"POST",headers:api._ah(),body:JSON.stringify(payload||{})}).then(r=>api._json(r)),
  vaultUpdateProject:(projectId,payload)=>fetch(`/api/vault/projects/${projectId}`,{method:"PUT",headers:api._ah(),body:JSON.stringify(payload||{})}).then(r=>api._json(r)),
  vaultIngest:(payload)=>fetch("/api/vault/ingest",{method:"POST",headers:api._ah(),body:JSON.stringify(payload||{})}).then(r=>api._json(r)),
  vaultRecords:(opts={})=>{
    const q=new URLSearchParams();
    if(opts.q)q.set("q",String(opts.q));
    ...
    if(opts.limit)q.set("limit",String(opts.limit));
    const qs=q.toString();
    return fetch(`/api/vault/records${qs?`?${qs}`:""}`,{headers:api._ah()}).then(r=>api._json(r));
  },
  vaultRecordDetail:(recordId,opts={})=>{
    const q=new URLSearchParams();
    if(opts.includeVersions)q.set("includeVersions","1");
    if(opts.versionsLimit)q.set("versionsLimit",String(opts.versionsLimit));
    const qs=q.toString();
    return fetch(`/api/vault/records/${recordId}${qs?`?${qs}`:""}`,{headers:api._ah()}).then(r=>api._json(r));
  },
  vaultCreateRecord:(payload)=>fetch("/api/vault/records",{method:"POST",headers:api._ah(),body:JSON.stringify(payload||{})}).then(r=>api._json(r)),
  vaultUpdateRecord:(recordId,payload)=>fetch(`/api/vault/records/${recordId}`,{method:"PUT",headers:api._ah(),body:JSON.stringify(payload||{})}).then(r=>api._json(r)),
  vaultRevealRecord:(recordId)=>fetch(`/api/vault/records/${recordId}/reveal`,{method:"POST",headers:api._ah()}).then(r=>api._json(r)),
  vaultSubscriptions:(opts={})=>{
    const q=new URLSearchParams();
    if(opts.days)q.set("days",String(opts.days));
    if(opts.limit)q.set("limit",String(opts.limit));
    const qs=q.toString();
    return fetch(`/api/vault/subscriptions${qs?`?${qs}`:""}`,{headers:api._ah()}).then(r=>api._json(r));
  },
  vaultSummary:()=>fetch("/api/vault/summary",{headers:api._ah()}).then(r=>api._json(r)),
  semantic:(id,opts={})=>{
    const q=new URLSearchParams();
  ...
  createApiToken:(payload)=>fetch("/api/auth/token",{method:"POST",headers:api._ah(),body:JSON.stringify(payload||{})}).then(r=>api._json(r)),
  listApiTokens:()=>fetch("/api/auth/tokens",{headers:api._ah()}).then(r=>api._json(r)),
